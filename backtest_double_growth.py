...
回测规则：
1. 调仓频率：每季度最后一个交易日
2. 选股逻辑：复用 double_single_growth_selector.py 的 select_double_growth
...
class DoubleGrowthBacktest:
    ...
    def run(self) -> pd.DataFrame:
        ...
        for i, rebalance_dt in enumerate(rebalance_dates):
            ...
            # ---------- 1. 选股 ----------
            try:
                df_selected = self.selector.select_double_growth(self.criteria, trade_date=rd_str)
...
class BacktestAnalyzer:
    ...
    def plot(self, output_path: str = 'backtest_double_growth.png'):
        """绘制收益曲线"""
        ...
        ax1.set_title('双增策略季度回测 — 累计收益率', fontsize=14)
...
def main():
    ...
    # 双增选股参数（与 double_single_growth_selector.py 默认一致）
    criteria = GrowthCriteria(
        strategy='double',
        max_peg=0.5,
        min_profit_growth=0.50,
        min_revenue_growth=0.10,
        max_revenue_growth=None,
        max_deduct_pe=50.0,
    ...
    try:
        ...
        # 保存结果
        df_daily.to_csv('backtest_double_growth_daily.csv', encoding='utf-8-sig')
        logger.info("每日净值已保存: backtest_double_growth_daily.csv")

        if backtest.trade_records:
            pd.DataFrame(backtest.trade_records).to_csv(
                'backtest_double_growth_trades.csv', index=False, encoding='utf-8-sig'
            )
            logger.info("交易记录已保存: backtest_double_growth_trades.csv")
        ...
        if backtest.rebalance_records:
            ...
            df_rebalance.to_csv('backtest_double_growth_rebalance.csv', index=False, encoding='utf-8-sig')
            logger.info("调仓记录已保存: backtest_double_growth_rebalance.csv")

        # 绘图
        analyzer.plot('backtest_double_growth.png')

...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('stock_history.log'), logging.StreamHandler()]
)
...
def fetch_stock_data(symbol: str, period: str, start_date: str,
    ...
    """
    用 tushare 的 daily 接口替换 akshare
    注意：
    1. tushare 的股票代码要带后缀 .SZ/.SH
    2. daily 接口本身只有「不复权」数据，复权请用 pro_bar（本例保持与原脚本一致，只取不复权）
    3. 返回字段直接按原 akshare 的列名做映射，后续 process_data 里无需再改
    """
    ...
    # 只保留需要的字段，并一次性重命名为原 akshare 的列名，这样下游 process_data 无需改动
    rename_cols = {
        'trade_date': '日期',
        'open': '开盘',
        'high': '最高',
    ...
    # 补充两个 akshare 里存在但 tushare 没有的字段，用 0 占位
    df['振幅'] = 0.0
    df['换手率'] = 0.0
...
def _ensure_ts_code(code: str) -> str:
    """6 位数字 -> 000001.SZ / 600000.SH"""
    ...
    def get_all_stocks(self) -> List[str]:
        try:
            ...
            stock_list = [
                ...
                if str(row['symbol']).startswith(('0', '3', '6'))
            ]

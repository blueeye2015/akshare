...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('stock_indicator_robust.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class RobustStockIndicatorCollector:
    def __init__(self, db_params: dict, max_workers: int = 3):
        """初始化健壮版数据采集器
        
        Args:
            db_params: 数据库连接参数
            max_workers: 最大并发工作线程数（降低以减少API压力）
        """
    ...
    def _create_session(self):
        ...
        # 设置超时
        session.timeout = (15, 45)  # (连接超时, 读取超时)
    ...
    def _smart_delay(self, base_delay: float = 2.0, jitter: float = 1.0):
        """智能延时，增加基础延时避免API限流"""
        delay = base_delay + random.uniform(0, jitter)
        time.sleep(delay)

    def _validate_dataframe(self, df, symbol: str) -> bool:
        """验证DataFrame的有效性"""
        try:
            if df is None:
            ...
            if df.empty:
                logger.warning(f"股票 {symbol} 返回空DataFrame")
                return False
            ...
            if df['trade_date'].isna().all():
                logger.warning(f"股票 {symbol} 所有日期都是无效的")
                return False
                
            return True
            
        except Exception as e:
            logger.warning(f"验证股票 {symbol} 数据时发生错误: {str(e)}")
            return False

    @retry(stop_max_attempt_number=2, wait_exponential_multiplier=3000, wait_exponential_max=20000)
    ...
    def retry_failed_stocks(self, mode: str = 'incremental'):
        """重试失败的股票"""
        ...
        for symbol in failed_stocks:
            try:
                logger.info(f"重试股票 {symbol}...")
                result = self.process_single_stock(symbol, mode)
                
                if result['status'] == 'success':
                    logger.info(f"重试股票 {symbol} 成功")
                else:
                    logger.warning(f"重试股票 {symbol} 仍然失败: {result['message']}")
                    
                # 重试间隔
                time.sleep(3)
                
            except Exception as e:
                logger.error(f"重试股票 {symbol} 时发生异常: {str(e)}")
...
def main():
    ...
    # 创建健壮版采集器实例（降低并发数）
    collector = RobustStockIndicatorCollector(db_params, max_workers=3)

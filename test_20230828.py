...
# 假设的新持仓
new_symbols = ['300059', '300750', '002594', '000725', '002415']  # 东财、宁德、比亚迪、京东方、海康
...
# 获取指数走势
engine = get_db_engine()
df_index = pd.read_sql(f"SELECT open, high, low, close FROM stock_history WHERE symbol='000001' AND trade_date='{REBALANCE_DATE}'", engine)
if not df_index.empty:
    index_open = float(df_index.iloc[0]['open'])
    index_high = float(df_index.iloc[0]['high'])
    index_low = float(df_index.iloc[0]['low'])
    index_close = float(df_index.iloc[0]['close'])
    index_change = (index_close - index_open) / index_open * 100
    
    print(f"\n📊 上证指数 {REBALANCE_DATE}:")
    print(f"   开盘: {index_open:.2f}")
    print(f"   最高: {index_high:.2f} (+{(index_high-index_open)/index_open*100:.2f}%)")
    print(f"   最低: {index_low:.2f} ({(index_low-index_open)/index_open*100:.2f}%)")
    print(f"   收盘: {index_close:.2f} ({index_change:+.2f}%)")
    
    if index_open > index_close and (index_high - index_open)/index_open > 0.01:
        print(f"   ⚠️  典型墓碑线形态: 高开低走，长上影线")
...
for d in sell_details:
    close_p = get_price(d['symbol'], REBALANCE_DATE, 'close')
    if close_p:
...
print(f"\n✅ 墓碑线换仓逻辑验证:")
print(f"   1. 当天高开 → 卖出旧持仓价格较高 ✓")
print(f"   2. 当天低走 → 买入新持仓后价格下跌（不利）✗")
print(f"   3. 但如果新持仓跌得比旧持仓少，换仓仍然划算")
